...
/*   Updated: 2021/03/31 13:11:28 by amuriel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_wall(t_engine *engine)
{
	...
	if (engine->side == 0)
	{
		if (engine->step_x > 0)
			...
			ft_get_pixel(&engine->data_so,
			engine->texture_x, engine->texture_y));
		else
			my_mlx_pixel_put(&engine->data, engine->x, engine->y,
			ft_get_pixel(&engine->data_no,
			engine->texture_x, engine->texture_y));
	}
	else
	{
		if (engine->step_y > 0)
			my_mlx_pixel_put(&engine->data, engine->x, engine->y,
			ft_get_pixel(&engine->data_ea,
			engine->texture_x, engine->texture_y));
		else
			my_mlx_pixel_put(&engine->data, engine->x, engine->y,
			ft_get_pixel(&engine->data_we,
			engine->texture_x, engine->texture_y));
	}
}
