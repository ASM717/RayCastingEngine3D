...
void	ft_sort_swap(t_engine *engine)
{
	...
	i = 0;
	engine->spriteDist = (double *)malloc(sizeof(double *) * engine->spriteNum);
...
void 	ft_main_sprites(t_engine *engine)
{
	...
	s = 0;
	ft_sort_swap(engine);
	while (s < engine->spriteNum)
	{
		engine->spriteX = engine->strSpr[engine->spriteOrder[s]].xStr - engine->posX;
		engine->spriteY = engine->strSpr[engine->spriteOrder[s]].yStr - engine->posY;
		...
		while (engine->stripe < engine->drawEndX)
		{
			...
			engine->scrWidth && engine->transformY < engine->zBuff[engine->stripe])
			{
				...
				{
					engine->tmpSprD = (engine->tmpSprY) * 256 - engine->scrHeight * 128 +
							engine->spriteH * 128;
